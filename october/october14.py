...
class FigureA:
    ...
    def setup(self, x: int, y: int, n=7): 
        ...
        """
        # off = 0.4 #* self.u * random()
        u = self.u
        root = self.root = V(x, y) # + V(random(), random()) * 0.2 * self.u
...
for fig, (x, y) in zip(shuffle_cycle(figs), grid_points(rows, cols, u)):
    fig.config(u, False)
    fig.setup(x, y, choice([3, 5, 7]))
    fig.draw(canvas.img)
